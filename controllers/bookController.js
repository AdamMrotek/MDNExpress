...
exports.book_create_post = [
  ...
  (req, res, next) => {
    if (!(req.body.genres instanceof Array)) {
      if (typeof req.body.genre === "undefined") req.body.genres = [];
...
exports.book_delete_get = function (req, res) {
  ...
  const getDetails = async () => {
    let booksInstances = BookInstance.find({ book: req.params.id });
    let book = Book.findById(req.params.id);
    let bookDetails = await Promise.all([booksInstances, book]);
    return { books_instances: bookDetails[0], book: bookDetails[1] };
  };
...
exports.book_update_get = function (req, res) {
  const getBookDetails = async () => {
    let bookPr = Book.findById(req.params.id)
      .populate("author")
      .populate("genre");
